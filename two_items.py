...
#-----------Parameters of Recalling Signal--------
A_recalling_ext = 0.145
B_recalling_ext = 1.8
T_recalling = 0.24
sig2 = 0.12
sig1 = 1.9
...
def simulate(m):
    ...
    for subj in range(num_sub):
        for tri in range(num_trial):
            ...
            diff = np.mod(pos - theta_recall + np.pi / 2, np.pi) - np.pi / 2
            a_ext_recalling = A_recalling_ext * np.cos(B_recalling_ext * diff) * (np.abs(B_recalling_ext * diff) <= np.pi / 2)
            NT1 = int((T_loading1 + T_gap + T_loading2 + T_maintaining[m]) / dt)
            NT2 = int((T_loading1 + T_gap + T_loading2 + T_maintaining[m] + T_recalling) / dt)
            I_ext[:, NT1:NT2] = np.tile(a_ext_recalling, (NT2 - NT1, 1)).T
